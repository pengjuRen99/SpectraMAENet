...
def get_args_parser():
    ...
    parser.add_argument('--loadWeight_path', default='param/200.pth', type=str)
...
def main(args):
    ...
    Input = np.load('dataset/original_npy/Test_Inputs.npy')
    Output = np.load('dataset/original_npy/Test_Outputs.npy')
    ...
    model.load_state_dict(torch.load(args.loadWeight_path, map_location=device))
...
def evaluate(dataloader, model, args, Input):
    ...
    with torch.no_grad():
        ...
        np.save('output_npy/SMAE_pretrain_denoised.npy', prediction)
