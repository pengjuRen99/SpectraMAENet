...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, spectra, truespectra, mask_ratio=0.75):       # denoising funetune - truespectra
        latent, mask, ids_restore = self.forward_encoder(spectra, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)
        loss = self.forward_loss(truespectra, pred, mask)
...
def spectraMAE_patch50_dim128_h4(**kwargs):     # denoising
    model = MaskedAutoencoderViT(
        spectra_size=500, patch_size=50, encoder_dim=128, depth=4, num_heads=4, decoder_embed_dim=128, 
        decoder_depth=4, decoder_num_heads=4, dim_mlp=256, 
    )
...
spectraMAE_base_patch50 = spectraMAE_patch50_dim128_h4
