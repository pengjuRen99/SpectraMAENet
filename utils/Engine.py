...
class Engine:
    ...
    @staticmethod
    def train_finetune(epoch, dataloader, model, optimizer, sheduler, lossF, device, val_acc, args):
        losses = AverageMeter()
        ...
        for batch, (data, target) in enumerate(bar):
            ...
            bar.set_description(
                'Epoch %d: Average train loss is %.6f, the accuracy rate of trainset is %.4f, learning rate is %.6f'
                % (epoch, losses.avg, accuracies.avg, optimizer.state_dict()['param_groups'][0]['lr'])
            )
        if args.sheduler_type=='CosineAnnealingLR':
                sheduler.step(epoch + batch / iter - 1)
        elif args.sheduler_type=='ReduceLR0nPlateau':
            sheduler.step(val_acc)
